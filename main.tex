...
% Could add images of all nine objects
...
\begin{center}
    ...
    \begin{minipage}{0.45\textwidth}
        ...
        \captionof{figure}{Sceenshot of UI Labels overlaid onto 360-image in Unity.}
        \label{360-labels}
    \end{minipage}\hfill
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=1\textwidth]{images/360-event-component.png}
        \captionof{figure}{Sceenshot of the Event Trigger component attached to each UI label in the 360-environment.}
        \label{360-labels}
    \end{minipage}
\end{center}


In the 360-image environment it is these labels that have event trigger components, illustrated in Figure \ref{360-labels}. After a user selects a UI label, the material of the sphere changes to the image which showcases that specific element.

In the computer-generated environment it is the objects themselves, or the hatches to those objects which a user interactes with. Each of the interactable objects have colliders, and event trigger components. An additional feature of the CG environment is that the hatched are animated.  
\\
\begin{lstlisting}
